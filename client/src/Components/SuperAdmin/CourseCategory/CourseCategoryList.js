...
import s from "./CourseCategoryList.module.css";
...
export default function CourseCategoryList() {
  ...
  const fetchRows = useCallback(() => {
    ...
    apiServiceHandler('GET', `course-category/list-pagination?${params}`)
      .then(res => {
  ...
  function doDelete() {
    ...
    apiServiceHandler('GET', `course-category/delete/${id}`)
      .then(() => { toast.success('Category deleted.'); fetchRows(); })
  ...
  function doBulkDelete() {
    ...
    const ids = [...selected];
    setSelected([]);
    Promise.all(ids.map(id => apiServiceHandler('GET', `course-category/delete/${id}`)))
      .then(() => { toast.success(`${ids.length} categor${ids.length !== 1 ? 'ies' : 'y'} deleted.`); fetchRows(); })
  ...
  return (
    <SuperAdminShell activeSection="course-category">
      ...
      <ConfirmModal
        ...
        message={`Delete ${selected.length} selected categor${selected.length !== 1 ? 'ies' : 'y'}? This cannot be undone.`}
      ...
      <div className={s.pageHeader} style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', width: '100%', gap: '12px' }}>
        <div>
          <h1 className={s.pageTitle}>Course Categories</h1>
          <p className={s.pageSubtitle}>Manage course categories</p>
        </div>
        <button className={s.btnAdd} onClick={() => router.push('/superadmin/course-category/add')}>
          + Add Category
        </button>
      </div>
                ...
                <tr key={row._id} style={{ cursor: 'pointer' }} onClick={() => toggleOne(row._id)}>
                  ...
                  <td>
                    <div className={s.nameCell}>
                      <div className={s.catIcon}>{Icon.cat}</div>
                      <span className={s.catName}>{row.title ?? '—'}</span>
                    </div>
                  </td>
                  <td>{row.desc ?? '—'}</td>
                  <td>
                  ...
                  <td>
                    <div className={s.actions} onClick={e => e.stopPropagation()}>
                      ...
                      <button className={s.btnEdit} title="Edit"
                        onClick={() => router.push(`/superadmin/course-category/${row._id}/edit`)}>
                        {Icon.edit}
                      </button>
        ...
        <div className={s.tableFooter}>
          <div className={s.footerLeft}>
            ...
            <span>
              {total === 0
                ? 'No categories'
                : `Showing ${from} to ${to} of ${total} categor${total !== 1 ? 'ies' : 'y'}`}
            </span>
          </div>
